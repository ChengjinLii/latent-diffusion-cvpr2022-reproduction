...
需要注意，本文没有复现完整 ImageNet-val reconstruction FID，也没有对 CelebA-HQ 生成样本计算 5k--50k FID。严格复现论文 Fig. 5、Fig. 6 或 Table 1/3 需要大规模训练、真实数据集统计和大量生成样本。本文完成的是小规模但端到端的核心机制验证：latent autoencoder 在较低维空间中保持足够高的视觉重建质量，官方 LDM checkpoint 能在 latent 空间通过 DDIM 快速采样，且采样步数直接决定速度成本。
